...
restart_apache () {
	printf '\e[0;31m\nRestarting Apache...\e[0m\n'
	sudo systemctl restart apache2
	printf '\e[0;32mApache restarted.\n\n'
}
...
mapfile -t versions < <(update-alternatives --list php | grep -oP "(\d\.\d+?)") || true
...
switch_from=$(php -v | grep -oP "PHP (\d\.\d+?)" | cut -d " " -f2) || true
